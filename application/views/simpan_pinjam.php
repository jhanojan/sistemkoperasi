<span style="font-size:24px; margin-bottom:5%; margin-top:5%;"><?php echo $title;?></span>
...
{
    if (com == 'add' ) {
		window.location = _base_url + controller + 'detail/';
    }
    if (com == 'sellect' )
    {
	...
	if(com=='detail'){
		...
		//var items = $('.trSelected',grid);
 		 window.location = _base_url + controller + 'detail/' + $('td:nth-child('+ (1+$.inArray('idnya',abbr)) +')>div', '.trSelected',grid).text();
			} else {
				return false;
			} 
	}
           ...
           if($('.trSelected',grid).length>0){
			   if(confirm('Delete ' + $('.trSelected',grid).length + ' items?')){
    			...
    			var abbr = [];
					...
					$.ajax({
					   type: "POST",
					   url: "<?php echo site_url($filename."/deletec");?>",
					   data: "items="+itemlist,
