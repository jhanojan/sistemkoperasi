...
if (!function_exists('GetValue')){
	function GetValue($field,$table,$filter=array(),$order=NULL)
	{
		...
		$q = $CI->db->get($table) or die(''.lastq());
		foreach($q->result_array() as $r)
		{
			return $r[$field];
		}
...
if (!function_exists('GetColumns')){	
	...
	{
		$CI =& get_instance();
		if(substr($tbl,0,3) != "tb_") $tbl = "tb_".$tbl;
...
if (!function_exists('GetOptAll')){
	function GetOptAll($tabel,$judul=NULL)
	{
		...
		foreach($q->result_array() as $r)
		{
			if(!isset($r['title'])) $r['title']=$r['nama'];
			$opt[$r['id']] = $r['title'];
		}
...
if (!function_exists('GetOptAllYes')){
	function GetOptAllYes($tabel,$judul=NULL)
	{
		if($tabel == "pendidikan") $filter = array("urut"=> "order/asc");
		else $filter = array();
		$filter=array('status'=>'where/y');
		$q = GetAll($tabel, $filter);
		if($judul) $opt[''] = $judul;
		foreach($q->result_array() as $r)
		{
...
if (!function_exists('GetOptAllYesCus')){
	function GetOptAllYesCus($tabel,$val,$judul=NULL)
	{
		$filter=array('status'=>'where/y');
		$q = GetAll($tabel, $filter);
		if($judul) $opt[''] = $judul;
...
if (!function_exists('GetOptAllYesCuss')){
	function GetOptAllYesCuss($tabel,$val,$val2,$judul=NULL)
	{
		$filter=array('status'=>'where/y');
		$q = GetAll($tabel, $filter);
		if($judul) $opt[''] = $judul;
...
if (!function_exists('GetOptMenu')){	
	function GetOptMenu()
	{
		...
		foreach($q->result_array() as $r)
		{
			$opt[$r['id']] = str_replace('0','Menu Utama',GetValue('title','tb_menu_admin',array('id'=>'where/'.$r['id_parents']))).' - '.$r['title'];
		}
