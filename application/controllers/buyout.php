...
class Buyout extends CI_Controller 
{	
	var $filename = "buyout";
	var $tabel = "tb_pembelian_detail";
	var $id_primary = "id";
	var $title = "Pembelian";
	...
	function detail($id=null){
		...
		$data['supplier'] = GetOptAll('tb_supplier','-Supplier-');
	...
	function update()
	{
		...
		foreach($GetColumns as $r)
		{
			...
			$bln=date('m');
			$thn=date('Y');
			$qs=$this->db->select('*')->from('tb_seri_pembelian')->where('tgl',date("Y-m-d"))->get();
			$cekpembelian=$qs->num_rows();
			if($cekpembelian==0){
			$seri=0;}
			else{
			$series=$qs->row_array();
			$seri=$series['pembelian'];	
			}
			$urut=substr(100000+$seri+1,1);
			$data['pj']['id_supplier']=$this->input->post('supplier');
			$data['pj']['ppn']=$this->input->post('ppn');
			$data['pj']['diskon']=$this->input->post('diskon');
			$data['pj']['id_pembelian']=$kodekop.date("dm").'-'.substr($thn,-2).'-'.$urut;
			$data['pj']['tanggal']=$now;
			$data['pj']['kasir']=$this->session->userdata('admin');
			$harga_dasar=0; 
			$data['pj']['total']=$this->input->post('totalprice');
			$kas=cari_kas('Pembelian');
				$catat=debit_kas('Pembelian',$data['pj']['id_pembelian'],$data['pj']['total'],$kas);
				if($catat==FALSE){
				$this->session->set_flashdata("message", 'Pembelian Gagal! Saldo Kas Tidak Mencukupi');
			...
			$data['pj_detail'][$s]['id_pembelian']=$data['pj']['id_pembelian'];
			...
			$this->db->insert('tb_pembelian_detail',$data['pj_detail'][$s]);
		...
		else
		{
			...
			$this->db->insert('tb_pembelian',$data['pj']);
			if($cekpembelian==0){
			$this->db->insert('tb_seri_pembelian',array('tgl'=>date('Y-m-d'),'pembelian'=>1));
			
			}
			else{
			$this->db->where('tgl',date('Y-m-d'));
			$this->db->update('tb_seri_pembelian',array('pembelian'=>$seri+1));
			}
		}
