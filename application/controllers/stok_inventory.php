...
class Stok_inventory extends CI_Controller 
{	
	var $filename = "stok_inventory";
	var $tabel = "tb_inventory_jumlah";
	var $id_primary = "id";
	var $title = "List Jumlah Inventory";
	...
	function detail($id=null){
		...
		$data['opt_bul']=GetOptMonth();
		$data['opt_year']=GetOptYear();
	...
	function update()
	{
		...
		$data['masuk_oleh'] = $this->session->userdata('admin');
		$data['stok']=GetValue('jumlah','tb_inventory',array('kode_barang'=>'where/'.$data['kode_barang']));
		//print_mz($data);
		if($id > 0)
		{
	...
	function get_column(){
            ...
            $colModel['tgl'] = array('Tanggal',50,TRUE,'left',2);
            $colModel['kode_barang'] = array('Kode Barang',100,TRUE,'left',2);
            $colModel['stok'] = array('Stok',100,TRUE,'left',2);
			$colModel['fisik'] = array('Fisik',100,TRUE,'left',2);
        	$colModel['selisih'] = array('Selisih',100,TRUE,'left',2);
            $gridParams = array(
                'rp' => 25,
	...
	function get_record(){
            ...
            $valid_fields = array('id','bulan','tahun','kode_barang','jumlah');
