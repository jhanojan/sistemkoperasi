...
class kas_config extends CI_Controller 
{	
	var $filename = "kas_config";
	var $tabel = "tb_config_kas";
	var $id_primary = "id";
	var $title = "KAS yang digunakan";
	...
	function detail($id=null){
		...
		$data['isi_kas'] = GetOptAll('tb_kas','-Kas-');
	...
	function get_column(){
            ...
            $colModel['nama'] = array('Nama Transaksi',100,TRUE,'left',2);
            $colModel['kas'] = array('Kas',100,TRUE,'left',2);
	...
	function get_record(){
            ...
            $valid_fields = array('id','kas','value');
			...
			elseif($row->status=='s'){$status='Suspended';}*/
                ...
                $row->id,
                $row->item,
				GetValue('nama','tb_kas',array('id'=>'where/'.$row->kas))
